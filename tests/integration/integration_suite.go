...
func (s *UpdaterSuite) setMockResponse(resp *agentmanager.GetVersionResponse) {
	...
	req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, mockServerControlURL+"/api/response", bytes.NewReader(data))
...
func (s *UpdaterSuite) getLatestRequest() *agentmanager.GetVersionRequest {
	httpReq, err := http.NewRequestWithContext(context.Background(), http.MethodGet, mockServerControlURL+"/api/request/latest", nil)
	require.NoError(s.T(), err, "Failed to create request")
	...
	grpcReq := &agentmanager.GetVersionRequest{}
	err = protojson.Unmarshal(body, grpcReq)
	require.NoError(s.T(), err, "Failed to unmarshal request")
	return grpcReq
}

func (s *UpdaterSuite) getMockRequests() []*agentmanager.GetVersionRequest {
	httpReq, err := http.NewRequestWithContext(context.Background(), http.MethodGet, mockServerControlURL+"/api/requests", nil)
	require.NoError(s.T(), err, "Failed to create request")
	resp, err := http.DefaultClient.Do(httpReq)
	...
	reqs := make([]*agentmanager.GetVersionRequest, 0, len(rawMessages))
...
func (s *UpdaterSuite) clearMock() {
	req, err := http.NewRequestWithContext(context.Background(), http.MethodPost, mockServerControlURL+"/api/clear", nil)
	require.NoError(s.T(), err, "Failed to create request")
...
// adjustStarttimeAwk adjusts field 22 (starttime) in /proc/<pid>/stat so that
// gopsutil's CreateTime calculation stays correct when btime is faked.
const adjustStarttimeAwk = `awk -v offset=$OFFSET '{ ` +
	`idx = index($0, ") "); ` +
	`if (idx == 0) { print; next } ` +
...
// fakeSystemUptime sets up /fake_proc so gopsutil (HOST_PROC=/fake_proc) sees a fake boot time.
// The updater must be stopped before calling this. Returns a cleanup function.
func (s *UpdaterSuite) fakeSystemUptime(uptimeDuration time.Duration) func() {
	...
	script := fmt.Sprintf(
		`set -e && `+
			...
			`    `+adjustStarttimeAwk+` /proc/$pid/stat > /fake_proc/$pid/stat 2>/dev/null || true; `+
			`  fi; `+
			`done && `+
...
func (s *UpdaterSuite) copyProcPid(pid string) {
	script := fmt.Sprintf(
		`REAL_BTIME=$(grep '^btime' /proc/stat | awk '{print $2}') && `+
			...
			`OFFSET=$(( (FAKE_BTIME - REAL_BTIME) * CLK_TCK )) && `+
			`mkdir -p /fake_proc/%s && `+
			adjustStarttimeAwk+` /proc/%s/stat > /fake_proc/%s/stat 2>/dev/null || true`,
		pid, pid, pid)
